# Stage 1: Retinex Decomposition Pretraining
#
# Pretrain only the RetinexDecomposer on LOL/SICE paired data
# with reconstruction + smoothness + colour consistency losses.
# The detection backbone is frozen during this stage.

_base_ = ['../../_base_/default_runtime.py']

# Only the decomposer and light encoder — no detection components
model = dict(
    type='DarkDINO',
    num_queries=900,
    ...
    backbone=dict(
        ...
        backbone=dict(
            ...
            num_stages=4,
            out_indices=(1, 2, 3),
            frozen_stages=-1,  # freeze ALL stages for pretrain
    ...
    dn_cfg=dict(
        label_noise_scale=0.5, box_noise_scale=1.0,
        group_cfg=dict(dynamic=True, num_groups=None, num_dn_queries=100)),
    retinex_loss=dict(
        type='RetinexConsistencyLoss',
        recon_weight=10.0,   # higher weight during pretraining
        smooth_weight=1.0,
        color_weight=0.5),
